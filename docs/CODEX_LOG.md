...
### Prompt ID: Navigation v1: add Threads index, add breadcrumb/back links, make nav less confusing (commit: TODO)
#### Prompt
```text
[Prompt] Navigation v1: add Threads index, add breadcrumb/back links, make nav less confusing

Context
- Current header nav links are defined in statrumble/app/components/HeaderNavLinks.tsx (/, /decisions, /workspaces, /join).
- Home (/) already lists recent threads via listThreads(10).
- Thread detail page is statrumble/app/threads/[id]/page.tsx and currently lacks "back" navigation.
- Goal: make in-app navigation intuitive; reduce getting lost.

Goals
1) Add a dedicated /threads page that lists recent threads (similar UI to home threads list), and supports kind badge (Proposal).
2) Add "Threads" to the header nav.
3) Rename "/" nav label from "StatRumble" to "Arena" (keep href "/").
4) Hide "Join" in header when the user already has workspace membership (or when logged in and activeWorkspaceId exists). Keep Join CTA inside /workspaces hub.
5) Add simple breadcrumb/back links:
   - On thread page: "Back to Threads" (-> /threads) and "Back to Arena" (-> /#chart or /).
   - On decision detail page: add "Back to Threads" link near the existing "Back to list".

Implementation details
A) New page: statrumble/app/threads/page.tsx
- Server component, dynamic = "force-dynamic"
- If no membership: show onboarding card same style as /decisions page.
- Else: listThreads(50) and render list items:
  - Link to /threads/<id>
  - Badge if kind === "transform_proposal"
  - Show metric name/unit and created_at.

B) Header nav changes: statrumble/app/components/HeaderNavLinks.tsx
- Add { href: "/threads", label: "Threads", isActive: pathname.startsWith("/threads") }
- Change "/" label to "Arena"
- Make Join conditional:
  - Accept a prop like `showJoin?: boolean` and render Join only if true
  - Wire in layout to decide showJoin based on membership/workspaceSelection.

C) Layout wiring: statrumble/app/layout.tsx
- You already load workspaceSelection when user exists.
- Determine showJoin:
  - showJoin = !workspaceSelection.activeWorkspaceId (or workspaceSelection.workspaces.length === 0)
- Pass showJoin into HeaderNavLinks.

D) Thread detail back links: statrumble/app/threads/[id]/page.tsx
- At top of main, add a small row with:
  - Link to "/threads" labeled "Back to Threads"
  - Link to "/" labeled "Back to Arena"
- Keep existing content intact.

E) Decision detail back link: statrumble/app/decisions/[id]/page.tsx
- Add Link to "/threads" labeled "Back to Threads" next to existing "Back to list".
...
- ./scripts/verify.sh
- Manual:
  - Navigate: Arena -> Threads -> Thread -> Back to Threads works
  - Promote to Decision -> Back to Threads works
  - Join link hidden when membership exists, visible otherwise

Output
- Changed files list
- Brief notes on navigation behavior
- Verify results
- Suggested commit: feat(nav): threads index + breadcrumbs and cleaner header nav
```
#### Result
- Added new `/threads` index page with workspace-membership onboarding fallback and a recent threads list (`listThreads(50)`) including Proposal badge, metric/unit, and created timestamp.
- Updated header navigation to include `Threads`, rename `/` label to `Arena`, and conditionally show `Join` via `showJoin` prop.
- Wired `showJoin` in root layout to hide Join when a workspace is already selected or memberships exist.
- Added thread detail top links (`Back to Threads`, `Back to Arena`) and decision detail `Back to Threads` link next to `Back to list`.
#### Manual Checklist
- [x] `npm run lint`
- [x] `npm run typecheck`
- [x] `./scripts/verify.sh`
- [ ] Manual nav: Arena -> Threads -> Thread -> Back to Threads
- [ ] Manual nav: Decision detail -> Back to Threads
...
### Prompt ID: Hybrid vote config: workspace template + thread snapshot/override (owner-only, reset option) (commit: TODO)
#### Prompt
```text
[Prompt] Hybrid vote config: workspace template + thread snapshot/override (owner-only, reset option)

Context
- Workspace vote profiles already exist:
  - DB: public.workspace_vote_profiles + RPC get_workspace_vote_profile / set_workspace_vote_profile
  - App: /workspaces Vote settings UI
  - Thread UI currently resolves workspace vote profile and passes votePrompt/voteLabels to ThreadArena.
  - Judge route uses workspace vote profile for modelInput.vote_prompt / vote_labels.
- We want thread-level semantics:
  - Each thread must carry its own vote prompt + A/B/C labels (snapshot at thread creation time).
  - Owner can override per thread.
  - If any votes exist, prevent semantic drift: require reset to change (optional), otherwise reject.

Constraints
- No new dependencies.
- Keep UI text English only (Hangul 0).
- Follow existing Supabase patterns: security definer RPC for privileged operations.

Goals
1) Add per-thread vote config fields stored in arena_threads:
   - vote_prompt (text)
   - vote_labels (jsonb with A/B/C string values)
2) Snapshot at creation time:
   - When creating discussion thread (/api/threads/create), copy the workspace template (resolved defaults) into thread.vote_*.
   - When creating transform proposal thread (/api/threads/propose-transform), do the same using transform_proposal kind.
3) Override per thread:
   - Provide an owner-only RPC to update thread vote config.
   - If votes exist, reject unless reset_votes=true (then delete all votes for that thread).
   - When vote config changes, clear referee_report (to avoid stale report).
4) UI:
   - On thread page, show a "Thread vote settings" panel:
     - read-only for non-owners
     - editable for owners
     - has a "Reset existing votes" checkbox (required if votes already exist)
     - calls an API route which invokes the RPC
5) Referee input:
   - Judge route must use thread vote config first (authoritative), and fall back to workspace template only if thread fields are missing/invalid (legacy safety).

Tasks

A) DB migration (next sequential number after 020_*.sql, likely 021_thread_vote_profile_snapshot.sql)
1) Add columns to public.arena_threads:
   - add column if not exists vote_prompt text;
   - add column if not exists vote_labels jsonb;
2) Backfill existing threads so semantics are frozen going forward:
   - For rows where vote_prompt is null OR vote_labels is null:
     - If workspace_vote_profiles exists for that workspace, use:
       - section = (case when kind='transform_proposal' then 'transform_proposal' else 'discussion' end)
       - vote_prompt = wvp.config->section->>'prompt'
       - vote_labels = wvp.config->section->'labels'
     - If no workspace profile row exists, use hardcoded defaults (same as DEFAULT_VOTE_PROFILE_CONFIG):
       - discussion:
         - prompt: "Is the change in the selected range meaningful?"
         - labels: {"A":"Yes","B":"No","C":"Unclear"}
       - transform_proposal:
         - prompt: "Should we accept this transform proposal?"
         - labels: {"A":"Accept","B":"Reject","C":"Revise"}
3) After backfill, enforce invariants:
   - alter column vote_prompt set not null
   - alter column vote_labels set not null
   - Add a CHECK constraint (idempotent) verifying:
     - jsonb_typeof(vote_labels)='object'
     - has keys A,B,C
     - values are strings and non-empty (check jsonb_typeof(vote_labels->'A')='string', etc)
4) Add owner-only RPC:
   ...
   - set search_path = public, pg_temp
   - Behavior:
     - require auth.uid() (Unauthorized if null)
     - load thread workspace_id; if missing -> raise exception 'Thread not found.'
     - owner check: exists workspace_members where workspace_id=thread.workspace_id AND user_id=auth.uid() AND role='owner'
       -> else raise exception 'Forbidden.'
     - normalize prompt: trim, must be non-empty
     - validate labels jsonb: must be object with A/B/C string non-empty
     - count votes: select count(*) from arena_votes where thread_id=p_thread_id and workspace_id=thread.workspace_id
       - if count > 0 and p_reset_votes is false: raise exception 'Votes exist; enable reset_votes to change.'
       - if count > 0 and p_reset_votes is true: delete from arena_votes where thread_id=p_thread_id and workspace_id=thread.workspace_id
     - update arena_threads:
       - set vote_prompt = normalized prompt
       - set vote_labels = normalized labels
       - set referee_report = null
       - (if there is a referee_report_updated_at column, also set it null or now() accordingly; otherwise ignore)
5) Permissions:
   - revoke all on function public.set_thread_vote_profile(uuid, text, jsonb, boolean) from public;
   - grant execute to authenticated;
   - do NOT grant to anon.

B) Thread creation: snapshot workspace template into thread
1) Update /api/threads/create route:
   - file: statrumble/app/api/threads/create/route.ts
   - Load workspace vote profile with getWorkspaceVoteProfile(activeWorkspaceId) (try/catch -> null)
   - resolve defaults with resolveVoteProfileConfig(profile)
   - voteKind = 'discussion'
   - include in insert payload:
     - kind: "discussion" (explicit)
     - vote_prompt: resolvedConfig.discussion.prompt
     - vote_labels: resolvedConfig.discussion.labels
2) Update /api/threads/propose-transform route:
   - file: statrumble/app/api/threads/propose-transform/route.ts
   - Load workspace vote profile for metricImport.workspace_id (or activeWorkspaceId)
   - voteKind = 'transform_proposal'
   - include vote_prompt/vote_labels in the initial insert into arena_threads (before the update that sets transform_* fields)

C) Types + thread loading
1) Update statrumble/lib/db/threads.ts:
   - ThreadRow type must include:
     - vote_prompt: string
     - vote_labels: unknown
   - ArenaThread type should expose these fields
2) Add helpers to parse/resolve vote labels safely:
   - file: statrumble/lib/voteProfile.ts
   - add:
     ...
     - Logic:
       - if threadPrompt is a non-empty string and parseVoteLabels(threadLabels) is valid -> source="thread"
       - else -> source="workspace" (workspaceConfig is already resolved with defaults)

D) Thread page: use thread vote config first + add ThreadVoteSettings UI
1) Update statrumble/app/threads/[id]/page.tsx
   - Keep loading workspaceVoteProfile as today
   - Compute workspace voteConfig (resolveVoteProfileConfig(profile)[voteKind])
   - Compute effective vote config using resolveThreadVoteConfig(thread.vote_prompt, thread.vote_labels, workspaceVoteConfig)
   - Pass effective.prompt/effective.labels to <ThreadArena ... />
2) Determine isOwner for the thread.workspace_id:
   - Call getActiveWorkspaceSelection() (same pattern as decisions page)
   - Find workspace entry matching thread.workspace_id and check role === "owner"
3) Add a new client component:
   - statrumble/app/components/ThreadVoteSettings.tsx
   - Props:
     - threadId: string
     - isOwner: boolean
   ...
   - UI:
     - Card title: "Thread vote settings"
     - Helper: "This thread uses a snapshot of the vote prompt and labels. Owners may edit; changing after votes exist requires reset."
     - Inputs: Prompt + labels A/B/C
     - Checkbox: "Reset existing votes"
     - Save button
     - On save: POST to /api/threads/<id>/vote-profile with { vote_prompt, vote_labels, reset_votes }
     - On success: show "Saved." and router.refresh()
     - On error: show error text
     - If !isOwner: disable inputs + show "Only owners can edit."
4) Render <ThreadVoteSettings ... /> above ThreadArena.

E) API route for thread vote profile update
1) Add route:
   - statrumble/app/api/threads/[id]/vote-profile/route.ts
2) POST body:
   - vote_prompt: string
   - vote_labels: { A: string; B: string; C: string }
   - reset_votes?: boolean
3) Require user auth (same as votes route).
4) Call supabase.rpc("set_thread_vote_profile", {
     p_thread_id: id,
     p_vote_prompt: vote_prompt,
     p_vote_labels: vote_labels,
     p_reset_votes: Boolean(reset_votes),
   })
5) Error mapping:
   - "Unauthorized." -> 401
   - "Forbidden." -> 403
   - "Votes exist; ..." -> 409 (conflict) or 400 (acceptable) but prefer 409
   - default -> 500
6) Response:
   - { ok: true } on success

F) Referee (judge) route: use thread config first
1) Update statrumble/app/api/threads/[id]/judge/route.ts
   - Instead of always using workspace voteConfig.prompt/labels, compute effective config:
     - workspaceConfig = resolveVoteProfileConfig(workspaceVoteProfile)[voteKind]
     - effective = resolveThreadVoteConfig(thread.vote_prompt, thread.vote_labels, workspaceConfig)
   - Use effective.prompt and effective.labels in modelInput.vote_prompt / vote_labels

G) Docs + verification
1) Append entry to docs/CODEX_LOG.md:
   - Prompt text
   - Summary of schema + RPC + UI changes
   - Manual checklist
2) Run:
   - npm run lint
   - npm run typecheck
...
Manual checklist
- Create a new discussion thread: confirm ThreadArena shows the correct prompt/labels (from workspace template).
- Create a new transform proposal thread: confirm prompt/labels match transform_proposal template.
- As owner on a thread:
  - Change prompt/labels BEFORE any votes -> save succeeds, page refresh shows updated values.
  - Cast a vote, then attempt save WITHOUT reset -> gets 409/400 with clear message.
  - Save WITH reset -> votes cleared (counts become 0), referee_report cleared, config updated.
- As non-owner:
  - Inputs disabled and save is blocked; API returns 403 if attempted.

Suggested commit message
- feat(votes): snapshot vote prompt/labels per thread with owner override + reset
```
#### Result
- Added migration `statrumble/supabase/migrations/021_thread_vote_profile_snapshot.sql`:
  - adds `arena_threads.vote_prompt` and `arena_threads.vote_labels`,
  - backfills null rows from workspace profile by thread kind with hardcoded defaults fallback,
  - enforces `not null` on both columns,
  - adds idempotent `arena_threads_vote_labels_check` constraint for A/B/C non-empty string labels,
  - adds owner-only security-definer RPC `public.set_thread_vote_profile(...)` with vote-reset gate and `referee_report` clearing,
  - grants RPC execute to `authenticated` only.
- Updated thread creation routes to snapshot vote semantics at insert time:
  - `statrumble/app/api/threads/create/route.ts` now inserts explicit `kind: "discussion"` and `vote_prompt`/`vote_labels` from resolved workspace discussion config.
  - `statrumble/app/api/threads/propose-transform/route.ts` now inserts `vote_prompt`/`vote_labels` from resolved transform proposal config.
- Updated types/helpers:
  - `statrumble/lib/db/threads.ts` now includes `vote_prompt` and `vote_labels` on thread rows.
  - `statrumble/lib/voteProfile.ts` adds `parseVoteLabels` and `resolveThreadVoteConfig`, and reuses label parsing in profile validation.
- Added owner override flow:
  - New client component `statrumble/app/components/ThreadVoteSettings.tsx` (owner-editable prompt/labels, reset checkbox, save flow, non-owner lock state).
  - New API route `statrumble/app/api/threads/[id]/vote-profile/route.ts` calling RPC with auth + validation + error mapping (`401/403/404/409`).
  - `statrumble/app/threads/[id]/page.tsx` now resolves effective thread vote config, computes owner status, renders `ThreadVoteSettings` above `ThreadArena`, and passes effective vote prompt/labels to `ThreadArena`.
- Updated referee input behavior:
  - `statrumble/app/api/threads/[id]/judge/route.ts` now uses thread vote config first via `resolveThreadVoteConfig(...)`, then workspace fallback.
- Changed files:
  - `statrumble/supabase/migrations/021_thread_vote_profile_snapshot.sql`
  - `statrumble/app/api/threads/create/route.ts`
  - `statrumble/app/api/threads/propose-transform/route.ts`
  - `statrumble/app/api/threads/[id]/judge/route.ts`
  - `statrumble/app/api/threads/[id]/vote-profile/route.ts`
  - `statrumble/app/components/ThreadVoteSettings.tsx`
  - `statrumble/app/threads/[id]/page.tsx`
  - `statrumble/lib/voteProfile.ts`
  - `statrumble/lib/db/threads.ts`
#### Manual Checklist
- [x] `npm run lint`
- [x] `npm run typecheck`
- [x] `./scripts/verify.sh`
- [ ] Create a new discussion thread and verify prompt/labels snapshot from workspace discussion template
- [ ] Create a transform proposal thread and verify prompt/labels snapshot from transform_proposal template
- [ ] Owner: save prompt/labels before votes and verify persistence after refresh
- [ ] Owner: after votes exist, save without reset and verify conflict error
- [ ] Owner: save with reset and verify votes cleared + referee report cleared + new config applied
- [ ] Non-owner: verify settings are disabled and API returns 403 on attempted update
#### Commit Link
- TODO
