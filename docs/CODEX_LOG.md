...
#### Prompt
```text
Make the entire app UI English-only (remove all Korean), including date/time formatting.

Goal:
- ZERO Korean characters in user-facing UI.

Steps:

A) Find all Korean strings in UI
1) Run ripgrep search:
   (exclude node_modules)
2) For each match in statrumble/app/**:
   - Replace Korean UI strings with English equivalents.
   - Keep existing layout, just swap text.

B) Fix date/time formatting to avoid Korean output
3) Identify where timestamps are formatted (e.g., lib/formatDate.ts or similar).
4) Ensure all UI date/time uses a single deterministic formatter that outputs:
   "YYYY-MM-DD HH:mm" (24h) or "YYYY-MM-DD HH:mm:ss" if needed.
   - No locale-dependent strings.
   - No AM/PM words.
5) Update components that display timestamps (threads list, thread detail, import chart tooltips, range labels) to use that formatter.

C) Proposal UI texts
6) Ensure proposal entry form, fork form, errors, hints, labels are all English.
   Examples should be English prompts (e.g., "Clip outliers using IQR and apply MA(30)").
7) Ensure the "No meaningful change" hint (if present) is English.

D) Public pages
8) Check public portal pages (/portal, /p/*) and ensure no Korean appears.

E) Verification
9) npm run lint
10) npm run typecheck
11) ./scripts/verify.sh
    -> must return no matches for UI/public surfaces.
```
#### Result
- Replaced all Korean user-facing strings in `statrumble/app/**` with English text while preserving existing layout and flow.
- Updated public surfaces (`/portal`, `/p/w/[slug]`, `/p/decisions/[publicId]`) and proposal/collaboration UI text (forms, hints, errors, labels) to English-only.
- Reworked `statrumble/lib/formatDate.ts` into a deterministic, locale-independent formatter:
  - Date: `YYYY-MM-DD`
  - DateTime: `YYYY-MM-DD HH:mm:ss`
  - Fixed UTC+9 conversion without locale-dependent AM/PM output.
- Updated `scripts/verify-date-format.mjs` expected values to the new deterministic format.
- Removed Korean text in supporting scripts (`scripts/git_safe_push.sh`) and referee system prompt text in `app/api/threads/[id]/judge/route.ts`.
- Verified Korean character scan reports no matches in required paths.
#### Manual Checklist
- [x] `npm run lint`
- [x] `npm run typecheck`
- [x] `./scripts/verify.sh`
...
### Prompt ID: Improve Transform Proposal demo clarity (commit: TODO)
#### Prompt
```text
Improve Transform Proposal demo clarity:
1) Make clip effects visible by adding outliers_clipped to stats.
2) Make Compare-to-parent deltas readable: +/- sign, more precision, and “no meaningful change” hint.

Context:
- Transform engine: statrumble/lib/transforms/index.ts
  - applyTransform() returns stats with count_before/count_after/outliers_removed/mean/std/slope (+ warnings)
  - filter_outliers supports mode remove/clip (default clip)
- Thread UI renders proposal panel + compare-to-parent deltas in:
  statrumble/app/threads/[id]/page.tsx

Tasks:

A) Transform engine: add outliers_clipped
1) In statrumble/lib/transforms/index.ts:
   - Extend TransformStats to include:
     outliers_clipped: number
   - In filter_outliers implementation:
     - When mode === "clip": count how many points were actually clipped (value outside bounds) and increment outliers_clipped.
     - When mode === "remove": keep existing outliers_removed logic.
   - Ensure stats always includes outliers_clipped (default 0).
2) Update calculateSeriesStats signature/usage accordingly.
3) Update compareStats(a,b) to include delta for outliers_clipped (and keep existing deltas stable).
4) Update scripts/sanity-transforms.ts assertions:
   - For default clip case, assert outliers_clipped > 0 for a series with clear outliers.
   - Keep remove case assertions.

B) UI: show outliers_clipped and improve delta formatting
5) In statrumble/app/threads/[id]/page.tsx proposal panel:
   - Display outliers_clipped alongside outliers_removed.
   - Handle older threads where field may be missing (treat as 0 or show "—").
6) In “Compare to parent” section:
   - Render deltas as “child - parent”.
   - For float deltas (mean/std/slope): show sign + 4 decimals (e.g., +0.0123, -1.2345).
   - For count deltas: show sign + integer.
   - If all deltas are effectively zero (count deltas 0 and abs(float deltas) < 0.0001), show a small hint text:
     “This transformation shows little to no substantive change compared to the parent (the range/data may be benign or use an identical spec).”
7) Keep layout minimal; do not introduce heavy new components.
...
Output:
- Patch diff
- Short notes: how outliers_clipped is computed, how delta formatting changed
Suggested commit message:
feat(demo): show clipped outliers + improve compare deltas readability
```
#### Result
- Added `outliers_clipped` to transform stats and stats diff types in `statrumble/lib/transforms/index.ts`.
- Updated `filter_outliers` clip mode to count only points that were actually clipped (value outside computed bounds), while keeping remove-mode `outliers_removed` behavior unchanged.
- Updated `applyTransform`/`calculateSeriesStats` flow to always include `outliers_clipped` (default `0`).
- Extended `compareStats` to emit `outliers_clipped` delta without changing existing delta semantics.
- Updated `scripts/sanity-transforms.ts` to assert clipped default mode reports `outliers_clipped > 0` for a clear outlier series, while preserving remove-mode assertions.
- Updated `statrumble/app/threads/[id]/page.tsx`:
  - Proposal stats now render `outliers_clipped` alongside `outliers_removed` (legacy missing field falls back to `0`).
  - Compare section explicitly states deltas are `child - parent`.
  - Float deltas (`mean`, `std`, `slope`) now render signed values with 4 decimals.
  - Count deltas now render signed integers.
  - Added low-change hint when count deltas are zero and float deltas are under `0.0001`.
- Updated comparable-stats parsing in `statrumble/app/api/threads/propose-transform/route.ts` to support legacy stats payloads without `outliers_clipped` by defaulting to `0`.
#### Manual Checklist
- [x] `npm run lint`
- [x] `npm run typecheck`
- [x] `./scripts/verify.sh`
- [ ] Manual UI pass on `/threads/<id>` for new stats/delta rendering
#### Commit Link
- TODO

### Prompt ID: Convert repository to English-only (commit: TODO)
#### Prompt
```text
Goal: Make the entire repository English-only by removing ALL Korean (Hangul) characters from:
- UI strings (app pages/components)
- API responses/errors
- docs/ (including CODEX_LOG)
- code comments
- scripts and test fixtures
Also detect any filenames containing Hangul and handle them safely.
```
#### Result
- Created branch `chore/english-only-repo`.
- Inventory scan found Hangul content only in `docs/CODEX_LOG.md` (1138 matches) and no Hangul filenames.
- Removed remaining Hangul lines from `docs/CODEX_LOG.md` so repository content is English-only.
- Confirmed no Hangul filenames via both `find` and Python Unicode-range scan.
#### Manual Checklist
- [x] `npm run lint`
- [x] `npm run typecheck`
- [x] `./scripts/verify.sh`
