...
export async function POST(request: NextRequest, context: RouteContext) {
  ...
  try {
    ...
    const systemContent =
      "You are the Referee for a data debate. Use only the provided snapshot/votes/messages as evidence. Do not guess unknown facts; record uncertainty in confounders/next_checks and use verdict.leading=unclear when needed. data_facts must be directly traceable to provided numbers/text. Output JSON only and strictly follow the schema. Keep all string fields single-line; if needed, use escaped \\n sequences.";
