  ...
  return (
    <main className="mx-auto w-full max-w-6xl px-4 py-8 md:px-8">
      ...
      <section className="mt-4 rounded-lg border border-zinc-200 bg-white p-5">
        ...
        <p className="mt-1 text-sm text-zinc-600">Select an import and choose a range to create an Arena thread.</p>
        {importsError ? (
          <p className="mt-2 text-sm text-red-600">
            Load failed: {importsError instanceof Error ? importsError.message : "Unknown error"}
          </p>
        ) : (
      ...
      <section className="mt-4 rounded-lg border border-zinc-200 bg-white p-5">
        ...
        {threadsError ? (
          <p className="mt-2 text-sm text-red-600">
            Load failed: {threadsError instanceof Error ? threadsError.message : "Unknown error"}
          </p>
        ) : threads.length === 0 ? (
      ...
      <section className="mt-8 grid gap-4 md:grid-cols-2">
        <div className="rounded-lg border border-zinc-200 bg-white p-5">
          ...
          {metricsError ? (
            <p className="mt-2 text-sm text-red-600">
              Load failed: {metricsError instanceof Error ? metricsError.message : "Unknown error"}
            </p>
          ) : metrics.length === 0 ? (
            <p className="mt-2 text-sm text-zinc-600">None yet</p>
          ) : (
