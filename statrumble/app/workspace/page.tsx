...
import { getDefaultWorkspaceInvite } from "@/lib/db/workspaces";
...
export default async function WorkspacePage() {
  ...
  try {
    workspace = await getDefaultWorkspaceInvite();
  } catch (error) {
    ...
    <main className="mx-auto w-full max-w-6xl px-4 py-8 md:px-8">
      ...
      <section className="mt-6 rounded-lg border border-zinc-200 bg-white p-5">
        {loadError ? (
          ...
          <>
            <p className="text-sm text-zinc-600">Workspace: {workspace.name}</p>
          ...
          <>
            <p className="text-sm text-zinc-700">
              현재 기본 워크스페이스 멤버가 아닙니다. 초대 코드를 받아 먼저 Join 하세요.
            </p>
            <Link
              href="/join"
